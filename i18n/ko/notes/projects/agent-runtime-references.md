...
에이전트는 `agent-runtime/`을 index처럼 읽고, 사람은 원본 폴더를 계속 편집하면 돼요.

## 앞으로의 모양

장기적으로는 익숙한 이름을 그대로 써야 해요.
...
이 저장소에 도구별 시작 파일이 필요해지면 `AGENTS.md`나 `CLAUDE.md`도 같은 원본 규칙에서 생성하거나 연결하면 돼요.
