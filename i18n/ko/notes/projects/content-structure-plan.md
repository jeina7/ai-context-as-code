...
문서 묶음 노트는 이 시스템이 실제로 쓸모 있다는 걸 보여주는 맥락을 담아요.

## 현재 작성 기준

현재 노트는 이런 기준으로 작성했어요.

- 개인 Obsidian 노트에 기대지 않고 제품 방향을 설명해요.
- 결정은 채팅 기록이 아니라 오래 남는 맥락으로 보관해요.
- AI가 읽기 쉬운 영어를 기준 원본으로 둬요.
- 사람이 검토할 수 있도록 한국어 읽기용 문서를 제공해요.
- 모든 노트는 읽고, 연결하고, 수정하기 쉬운 크기로 유지해요.
- 개인 원본 자료는 이 저장소 안에서 독립적으로 읽힐 수 있을 때만 옮겨요.
...
- `projects/`는 제품과 도입 계획을 같이 담고 있어요.
- `decisions/`는 왜 특정 선택을 했는지 담아요.
- `research/`는 외부 흐름에서 얻은 현장 메모를 담아요.
- `worklog/`는 구현 이력을 담아요.

어색한 지점은 `projects/`예요.
제품 설계와 이전 계획이 섞여 있어요.
그래서 저장소가 실제로 쓰이는 맥락 제품이라기보다 일반 문서 보관함처럼 느껴질 수 있어요.

## 제안 폴더 구조

다음 구조는 제품, 방법, 문서 묶음을 분리해야 해요.
...
Obsidian 노트를 검토할 때는 이 기준을 써요.

- 제품 설계, 인터페이스, 로드맵, 시스템 동작은 `product/`로 가요.
- 에이전트가 직접 읽는 안내문, 명령, 스킬, 기억 참조는 `runtime/`으로 가요.
- 재사용 가능한 사고 패턴과 운영 규칙은 `method/`로 가요.
- 시스템이 실제로 쓸모 있다는 걸 보여주는 가져온 맥락은 `corpus/`로 가요.
...
이 구조는 모든 Obsidian 노트를 옮기지 않아요.
개인 생활 맥락을 이 저장소에 합치지 않아요.
조직 기능을 추가하지 않아요.
오픈소스 제품과 실제 개인 맥락 문서 묶음이 함께 자랄 수 있는 모양을 먼저 잡아요.
