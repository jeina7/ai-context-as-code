...
Markdown file이 source예요.
Build script는 source를 browser가 읽을 수 있는 data로 바꿔요.
Site는 사람과 AI agent가 탐색하고, 검토하고, 변경 초안을 만들 수 있는 공간을 제공해요.
...
Browser workspace는 변경 초안을 만들고 export할 수 있어요.
하지만 repository를 조용히 바꾸지 않아요.
이 경계는 local experiment 비용을 낮추면서도 배포 전 review를 지켜줘요.
