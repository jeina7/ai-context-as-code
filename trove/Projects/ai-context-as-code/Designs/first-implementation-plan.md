...
Cloudflare Pages와 Cloudflare Web Analytics는 첫 구현 범위에 포함해요.
...
- 편집 기능은 첫 구현에 넣지 않아요.
- Cloudflare Pages deploy output은 `dist/`예요.
- Cloudflare Web Analytics token은 source에 고정하지 않고 build-time config로 주입해요.
...
11. `site/` 정적 앱, `data/`, `_build/trove/` payload, `_redirects`를 `dist/`에 조립해요.
...
- `dist/_redirects`에는 `/trove/*`와 `/search`가 `index.html` app shell로 들어가는 규칙이 있어요.
...
## 7단계: Cloudflare Pages deploy 연결
...
| route fallback | `dist/_redirects`에서 `/trove/* /index.html 200`, `/search /index.html 200` |
| deploy blocker | validator error, duplicate ID, public payload safety error |
| preview deploy | Cloudflare Pages preview deployment를 사용해 main 배포 전 확인 |
...
- 기본 방식은 build-time token 주입이에요.
- token은 `ACAC_CF_WEB_ANALYTICS_TOKEN` 환경 변수에서 읽어요.
- token이 없으면 analytics script를 넣지 않고 `data/build.json`에 `analytics.enabled: false`를 기록해요.
- token이 있으면 `site/index.html`의 analytics placeholder에 Cloudflare beacon script를 주입해요.
- SPA 측정은 Cloudflare Web Analytics의 History API 추적에 맡겨요.
- Hash route를 쓰지 않기 때문에 `/trove/<id>` 이동이 문서별 page view로 잡힐 수 있어요.
- Cloudflare Pages dashboard의 one-click Web Analytics를 쓰는 경우에는 manual token injection과 중복으로 켜지 않아요.
...
- Cloudflare Pages output인 `dist/`가 생성돼요.
- Cloudflare Web Analytics token이 있으면 analytics script가 output에 들어가고, 없으면 비활성 상태가 build metadata에 남아요.
