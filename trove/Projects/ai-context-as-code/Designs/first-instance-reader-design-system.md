...
- `_config/`와 `_archived/`는 숨기지 않고 special contents로 구분해요.
...
| Warning | `--color-warning` | build warning, broken wikilink, special contents에 써요. |
...
| Module surface | Start here, Build status, Special contents 같은 의미 block |
...
- Brand, search, main context, special contents 순서로 둬요.
- `Daily/`와 `Projects/`는 main context예요.
- `_config/`와 `_archived/`는 divider 아래 special contents예요.
...
- Document metadata, table of contents, connection counts, backlinks, outgoing links, related context 순서로 보여줘요.
- Mobile과 tablet에서는 reader 아래로 내려가요.
- Graph view는 이번 범위가 아니에요.

### Search

- Search는 `/search?q=<query>` route를 써요.
- 결과는 title, path, summary, type/status/special label을 보여줘요.
- `_config/`와 `_archived/` 결과에는 special label을 붙여요.
...
- Search result에 special contents label이 붙고 `_assets/`는 보이지 않아요.
