...
방문자는 첫 화면에서 `trove/` 구조, 현재 프로젝트 예시, trove system layer가 어떻게 쓰이는지 바로 볼 수 있어야 해요.
...
| agent 사용자 | agent가 읽는 memory, skill, command 구조를 확인함 | `_config/`를 하단 trove layer로 보여주되 검색과 링크에는 포함해요. |
...
| Trove layers | `_config/Memory`, `_config/Skills`, `_config/Commands`가 무엇인지 짧게 설명 |
...
Obsidian처럼 폴더 tree를 보여주되, ACAC의 working context와 trove system layer를 시각적으로 분리해요.
...
| Working context | `Daily/`, `Projects/` |
| Trove layers | 구분선 아래 `_config/`, `_archived/` |
| Hidden storage | `_assets/`는 표시하지 않음 |

표시 규칙:

- `Daily/`와 `Projects/`는 항상 먼저 보여줘요.
- `_config/`와 `_archived/`는 구분선 아래 trove layer로 보여줘요.
...
- 화면의 1차 label은 `_config`, `_archived`가 아니라 `Operating layer`, `Archive`처럼 의미 기반 이름을 써요.
...
- `_config`, `_archived` 같은 trove layer는 색보다 위치와 의미 기반 label로 구분해요.
...
{
  "main": ["Daily", "Projects"],
  "system": ["_config", "_archived"],
  "hidden": ["_assets"]
}
```

이 grouping은 hard-coded UI 취향이 아니라 ACAC의 context 철학이에요.
working context와 trove system layer가 다르게 보이는 이유를 data에도 드러내요.
...
- sidebar에서 working context와 trove layers가 분리되어 보여요.
- `_assets/`는 보이지 않아요.
