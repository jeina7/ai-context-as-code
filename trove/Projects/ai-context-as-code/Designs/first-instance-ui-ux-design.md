...
방문자는 첫 화면에서 `trove/` 구조, 현재 프로젝트 예시, FORGE가 어떻게 쓰이는지 바로 볼 수 있어야 해요.
...
- jeina가 다음 작업을 이어받을 때 `Daily/`, `Projects/`, `forge/_config/` 중 어디를 봐야 하는지 바로 판단해요.
- 외부 방문자가 첫 인스턴스를 보고 ACAC가 어떤 제품이 될 수 있는지 상상할 수 있어요.
- agent-facing system layer가 숨겨진 설정 파일이 아니라 관리 가능한 markdown content로 보여요.
...
| agent 사용자 | agent가 읽는 memory, skill, command 구조를 확인함 | `forge/_config/`를 하단 FORGE layer로 보여주되 검색과 링크에는 포함해요. |
...
| Structure summary | `Daily/`, `Projects/`, `forge/_config/`, `forge/_archived/` 역할 |
...
| FORGE | `forge/_config/Memory`, `forge/_config/Skills`, `forge/_config/Commands`가 무엇인지 짧게 설명 |
...
Obsidian처럼 폴더 tree를 보여주되, ACAC의 TROVES와 FORGE를 시각적으로 분리해요.
...
| TROVES | `Daily/`, `Projects/` |
| FORGE | 구분선 아래 `forge/_config/`, `forge/_archived/` |
| Hidden storage | `forge/_assets/`는 표시하지 않음 |

표시 규칙:

- `Daily/`와 `Projects/`는 항상 먼저 보여줘요.
- `forge/_config/`와 `forge/_archived/`는 구분선 아래 FORGE layer로 보여줘요.
- `forge/_config/`는 접을 수 있지만 완전히 숨기지는 않아요.
- `forge/_archived/`는 기본 접힘 상태예요.
- `forge/_assets/`는 sidebar에 나오지 않아요.
- `visibility: private` 또는 `internal` 문서는 public build에서 sidebar에 나오지 않아요.
- 화면의 1차 label은 `forge/_config`, `forge/_archived`가 아니라 `Operating layer`, `Archive`처럼 의미 기반 이름을 써요.
- Folder row는 chevron과 row state로 구분하고, `dir`, `day`, `des` 같은 텍스트 badge와 folder/note icon은 쓰지 않아요.
...
- `forge/_assets/` 파일은 직접 검색 결과로 보여주지 않아요.
- `forge/_config/`와 `forge/_archived/`는 검색에 포함하되, 결과에서 layer label을 붙여요.
...
- `forge/_config`, `forge/_archived` 같은 FORGE layer는 색보다 위치와 의미 기반 label로 구분해요.
...
| `data/graph.json` | bounded relational context map preview |
...
TROVES와 FORGE가 다르게 보이는 이유를 data에도 드러내요.

`data/graph.json`은 read-only static contract로 시작해요.
첫 UI는 full interactive graph를 만들지 않고, 현재 문서 중심의 1-hop relation map preview만 보여줘요.

초기 계약:

| 필드 | 의미 |
|---|---|
| `nodes[]` | note id, title, type, status, visibility, route, source path, layer label |
| `edges[]` | `wikilink`, `backlink`, `folder`, `project` relation |
| `clusters.layers[]` | Daily, Projects, Operating layer, Archive 같은 표시용 묶음 |
| `contract.nonGoals[]` | full graph, runtime sync, live editor 같은 비목표 |
...
- 첫 화면에서 `Daily/`, `Projects/`, `forge/_config/`, `forge/_archived`의 역할을 이해할 수 있어요.
- sidebar에서 TROVES와 FORGE가 분리되어 보여요.
